...
public final class MysqlGenerator implements SqlGenerator {
    @Override
        ...
        for (int i = 0; i < tables.size(); i++) {
            ...
            for (ColumnDefinition column : table.columns()) {
                sql.append("ALTER TABLE ").append(tableName).append(" MODIFY COLUMN ")
                        .append(columnSql(column).trim()).append(" COMMENT '")
                        .append(escape(column.description())).append("';\n");
            }
    ...
    private static String type(ColumnDefinition column) {
        ...
        if (type.contains("CHAR") || type.equals("STRING")) return "VARCHAR(" + (size.isBlank() ? "255" : size) + ")";
